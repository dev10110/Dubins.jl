...
function dubins_LSL(intermediate_results::DubinsIntermediateResults{F}, loop_tol) where {F}
...
function dubins_RSR(intermediate_results::DubinsIntermediateResults{F}, loop_tol) where {F}
...
function dubins_LSR(intermediate_results::DubinsIntermediateResults{F}, loop_tol) where {F}
...
function dubins_RSL(intermediate_results::DubinsIntermediateResults{F}, loop_tol) where {F}
...
function dubins_RLR(intermediate_results::DubinsIntermediateResults{F}, loop_tol) where {F}
...
function dubins_LRL(intermediate_results::DubinsIntermediateResults{F}, loop_tol) where {F}
    ...
    if abs(tmp0) <= 1
        ...
        out2 = cleanup_loop(p, loop_tol)
        out3 =
            mod2pi(mod2pi(intermediate_results.β) - intermediate_results.α - t + mod2pi(p))
        out3 = cleanup_loop(out3, loop_tol)
        out = SVector(out1, out2, out3)
        return EDUBOK, out
    end
