...
"""
dubins_segment_length(path::DubinsPath, i::Int) =
    (i < 1 || i > 3) ? (return Inf) : (return path.params[i] * path.ρ)
...
"""
dubins_segment_length_normalized(path::DubinsPath, i::Int) =
    (i < 1 || i > 3) ? (return Inf) : (return path.params[i])
